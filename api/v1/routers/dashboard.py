...
async def get_control_metrics(
    ...
    for fdate, raw in funnel_result.all():
        ...
        for item in raw:
            ...
            if nm_id:
                ...
                product_totals["impressions"] += int(stat.get("openCount", 0) or 0)
                product_totals["clicks"] += int(stat.get("cartCount", 0) or 0)
    ...
    products_detail = await db.execute(
        select(
            ...
            TechStatus.barcode, TechStatus.target_date,
        ).where(
            TechStatus.organization_id == org_id,
            TechStatus.target_date >= d_from,
            TechStatus.target_date <= d_to,
    ...
    if has_funnel_summary:
        total_clicks = sum(p["clicks"] for p in funnel_products.values())
        total_impressions = sum(p["impressions"] for p in funnel_products.values())
    else:
