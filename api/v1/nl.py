...
async def get_cost_prices(org_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return [{"id": str(r[0]), "entity_id": str(r[1]) if r[1] else None, "nm_id": r[2], "barcode": r[3], "vendor_code": r[4],
             ...
             "shipment_method": r[56], "fbs_warehouse": r[57],
             "product_name": r[58]} for r in result.all()]
...
async def upload_cost_prices_excel(org_id: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    for row in rows:
        ...
        params = {
            ...
            "fbsw": ps(row, "Склад FBS", "fbs_warehouse"),
            # Прочее
    ...
    if (rows.length) {
        ...
        rows.forEach(row => {
            ...
            const cols = [nm, vc, bc, '"' + name + '"', size,
                ...
                gv('plan_length'), gv('plan_width'), gv('plan_height'), gv('plan_volume'), gv('plan_weight'),
