...
async def upload_cost_prices_excel(org_id: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        params = {
            ...
            # Классификация — нормализация A/B/C (отсечение мусора вроде «5», «Б», «В»)
            "pcls": (lambda v: v.upper() if v and v.upper() in ('A','B','C') else None)(ps(row, "Класс", "Класс товара", "product_class")),
